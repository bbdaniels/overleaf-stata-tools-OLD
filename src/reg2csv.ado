* Push regression to csv


cap prog drop reg2csv
prog def reg2csv

syntax ///
  anything        /// list of regressions
  using           /// location of file
  ,               /// options
  [colnames(string asis)] /// column titles
  [rownames(string asis)] /// row titles
  [stats(string asis)] /// stats to get
  [dec(integer 2)] /// number of decimal
  * /// xml_tab options such as p values
...
qui if "stats" != "" {
    foreach reg in `anything' {
      ...
      foreach stat in `stats' {
        local temp = `e(`stat')'
        mat regStats = nullmat(regStats) \ [`temp']
        mat nullStar = nullmat(nullStar) \ [0]
      }
      mat allStats = nullmat(allStats) , regStats
      mat allStars = nullmat(allStars) , nullStar
    }
    mat results = results \ allStats
    mat results_STARS = results_STARS \ allStars
  }

* Colnames

if `"`colnames'"' == "" {
  local colnames_old : colfullnames results
  foreach word in `colnames_old' {
      local colname = substr("`word'",1,strpos("`word'",":")-1)
...
if `"`rownames'"' == "" {
  local rownames_old : rowfullnames results
  foreach word in `rownames_old' {
      if regexm("`word'","_cons_easytofind0") {
        local rowname = "Constant"
        local rownames `"`rownames' "`rowname'""'
        }
        else if regexm("`word'","easytofind0") {
          local varname = substr("`word'",1,strpos("`word'","_easy")-1)
          local rowname : var label `varname'
          local rownames `"`rownames' "`rowname'""'
          }
        else if regexm("`word'","easytofind1") {
          local rowname = ""
          local rownames `"`rownames' "`rowname'""'
          }
    }
...
mat2csv results `using' ///
  , colnames(`colnames') rownames(`rownames' `stats') dec(`dec')
